...
unset($__errorArgs, $__bag); ?>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-0" style="background:#111; position:relative; min-height:200px; display:flex; align-items:center; justify-content:center;">
                    <img id="modalMainImg" src="" alt=""
                         style="display:block; max-height:60vh; max-width:100%; width:100%; object-fit:contain;">
                    <!-- prev photo -->
                    <button id="modalPrevBtn" type="button" onclick="changeModalPhoto(-1)"
                            style="display:none; position:absolute; left:0; top:0; bottom:0; background:rgba(0,0,0,0.35); border:none; color:#fff; padding:0 14px; font-size:1.4rem; cursor:pointer; z-index:2;">
                        <i class="fas fa-chevron-left"></i>
                    </button>
                    <!-- next photo -->
                    <button id="modalNextBtn" type="button" onclick="changeModalPhoto(1)"
                            style="display:none; position:absolute; right:0; top:0; bottom:0; background:rgba(0,0,0,0.35); border:none; color:#fff; padding:0 14px; font-size:1.4rem; cursor:pointer; z-index:2;">
                ...
                <div class="modal-footer d-flex justify-content-between align-items-center flex-wrap" style="padding:10px 15px; gap:8px;">
                    ...
                    <div class="d-flex align-items-center" style="gap:6px;">
                        <button type="button" class="btn btn-outline-secondary btn-sm" onclick="navigateProduct(-1)">
                            <i class="fas fa-chevron-right"></i>
                        </button>
                        <span id="modalProductCounter" style="font-size:0.82rem; color:#666; min-width:48px; text-align:center;"></span>
                        <button type="button" class="btn btn-outline-secondary btn-sm" onclick="navigateProduct(1)">
                            <i class="fas fa-chevron-left"></i>
                        </button>
                    </div>
    ...
    <script>
        ...
        let _modalPhotos         = [];
        let _modalIndex          = 0;
        let _currentProductIndex = 0;
        let _allModalProducts    = [];

        function viewProductPhotos(productId, event) {
            event.stopPropagation();

            _allModalProducts = productsData;
            _currentProductIndex = _allModalProducts.findIndex(p => p.id === productId);
            if (_currentProductIndex === -1) _currentProductIndex = 0;
        ...
        function navigateProduct(dir) {
            _currentProductIndex = (_currentProductIndex + dir + _allModalProducts.length) % _allModalProducts.length;
            _renderModalProduct();
        }

        function _renderModalProduct() {
            const product = _allModalProducts[_currentProductIndex];
            if (!product) return;

            _modalPhotos = (product.photos && product.photos.length > 0) ? product.photos : [product.image];
            _modalIndex  = 0;
            _renderModalPhoto();
            ...
            $('#modalProductCounter').text((_currentProductIndex + 1) + ' / ' + _allModalProducts.length);

            $('#modalBookedOverlay').css('display', product.booked ? 'flex' : 'none');

            const priceHtml = product.offer_price
                ? `<span style="text-decoration:line-through;color:#aaa;margin-inline-end:5px;">JD ${product.selling_price}</span>
        ...
        function _updateModalSelectBtn(product) {
