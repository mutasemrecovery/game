    ...
    <div class="container">
        ...
        <div class="search-section">
            <div class="search-container">
                <form method="GET" action="{{ route('showProducts') }}" class="search-form">
                    ...
                    @if($search)
                        <a href="{{ route('showProducts') }}" class="clear-btn">مسح البحث</a>
                    @endif
                </form>
                ...
                @if($search)
                    <div class="search-results-info highlight">
                        @if($products->count() > 0)
                            تم العثور على {{ $products->count() }} منتج للبحث عن "{{ $search }}"
                        @else
                            لم يتم العثور على نتائج للبحث عن "{{ $search }}"
                        @endif
                    </div>
                @else
                    <div class="search-results-info">
                        عرض جميع المنتجات ({{ $products->count() }} منتج)
                    </div>
                @endif
            </div>
        </div>
            ...
            <div class="products-grid" id="productsGrid">
                ...
                @foreach ($products as $product)
                    <div class="product-card">
                        <div class="product-image" onclick="openGallery({{ $product->id }}, '{{ $product->name_ar }}')">
                            <img src="{{ asset('assets/admin/uploads/' . $product->productImages->first()->photo) }}"
                                alt="{{ $product->name_ar }}" loading="lazy">
                            @if ($product->productImages->count() > 1)
                                <div class="image-counter">{{ $product->productImages->count() }} صور</div>
                            @endif
                        </div>
        ...
        @else
            <div class="no-results">
                ...
                @if($search)
                    <p>لم نتمكن من العثور على أي منتجات تطابق البحث "{{ $search }}"</p>
